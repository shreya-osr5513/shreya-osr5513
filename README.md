<div align="center">

<img
  src="./assets/hero_new.svg"
  width="100%"
  alt="Shreya Gupta — Third-year B.Tech ECE-AI Student at IGDTUW"
/>

<div align="center">

<a href="https://github.com/shreya-osr5513"><img src="https://img.shields.io/badge/shreya--osr5513-171D25?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"></a>
&nbsp;
<a href="https://www.linkedin.com/in/shreya-gupta-7b6b96250"><img src="https://img.shields.io/badge/shreya--gupta-315A76?style=for-the-badge&logo=data:image/svg%2Bxml;base64," alt="LinkedIn"></a>
&nbsp;
<a href="mailto:shreya.gupta.osr@gmail.com"><img src="https://img.shields.io/badge/shreya.gupta.osr-76594F?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"></a>

</div>

<br/><br/>

<a href="#02--experience-log">Experience</a>
&nbsp;•&nbsp;
<a href="#03--featured-builds">Projects</a>
&nbsp;•&nbsp;
<a href="#05--technology-matrix">Tech Stack</a>
&nbsp;•&nbsp;
<a href="#07--github-signals">GitHub Stats</a>

</div>

---

## `01 // PROFILE SNAPSHOT`

I enjoy building **backend-heavy applications** where APIs, databases, automation, and applied AI work together to solve practical problems.

<div align="center">

<img
  src="https://img.shields.io/badge/Backend_Development-2F6170?style=flat-square&logo=fastapi&logoColor=white"
  alt="Backend Development"
/>
<img
  src="https://img.shields.io/badge/Software_Engineering-405D78?style=flat-square&logo=codeforces&logoColor=white"
  alt="Software Engineering"
/>
<img
  src="https://img.shields.io/badge/AI_%2F_LLM_Applications-625F82?style=flat-square&logo=openai&logoColor=white"
  alt="AI and LLM Applications"
/>
<img
  src="https://img.shields.io/badge/APIs_%26_Data_Systems-496E70?style=flat-square&logo=databricks&logoColor=white"
  alt="APIs and Data Systems"
/>

</div>

<br/>

<details open>
<summary>
  <img
    src="https://img.shields.io/badge/WHAT_I_LIKE_BUILDING-6E6253?style=flat-square&logo=buffer&logoColor=white"
    alt="What I like building"
  />
</summary>

<br/>

Scalable backend workflows, clean REST APIs, automated systems, intelligent matching pipelines, and AI-powered product features.

</details>

<details>
<summary>
  <img
    src="https://img.shields.io/badge/CURRENTLY_STRENGTHENING-6E6253?style=flat-square&logo=target&logoColor=white"
    alt="Currently strengthening"
  />
</summary>

<br/>

`Data Structures & Algorithms` · `Backend Development` · `API Design` · `System Design`

</details>

<details>
<summary>
  <img
    src="https://img.shields.io/badge/HOW_I_APPROACH_A_PROBLEM-6E6253?style=flat-square&logo=blueprint&logoColor=white"
    alt="How I approach a problem"
  />
</summary>

<br/>

`Understand the workflow` → `Design the system` → `Build` → `Measure` → `Improve`

</details>

<br/>

<div align="center">

<img
  src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=20&duration=2800&pause=1000&color=82AFC2&center=true&vCenter=true&width=950&height=42&lines=Designing+APIs+that+keep+complex+workflows+simple.;Building+backend+systems+that+solve+real+problems.;Using+applied+AI+where+it+genuinely+improves+the+product.;Learning+deeply%2C+building+thoughtfully%2C+and+iterating+continuously."
  alt="Animated profile statement"
/>

</div>

---

## `02 // EXPERIENCE LOG`

<p align="left">
  <img
    src="https://upload.wikimedia.org/wikipedia/en/thumb/5/58/Indian_Institute_of_Technology%2C_Jammu_Logo.svg/250px-Indian_Institute_of_Technology%2C_Jammu_Logo.svg.png"
    width="72"
    align="left"
    alt="IIT Jammu logo"
  />

  <strong>IIT Jammu</strong><br/>
  <strong>Summer AI Intern</strong><br/>
  <code>Jun 2025 – Aug 2025</code>
</p>

<br clear="left"/>

> Built backend automation and intelligent job-matching workflows by combining APIs, LLMs, embeddings, and structured data pipelines.

- Built backend workflows integrating **LLMs, REST APIs, Gmail API, and Google Sheets API**
- Developed an **embedding-based job-matching system** for comparing candidate profiles with job listings
- Automated structured tracking of **100+ job listings**
- Used **asynchronous API calls** to reduce blocking across repeated data-collection and matching workflows

### Technologies Used

<div align="left">

<img
  src="https://img.shields.io/badge/Python-202B36?style=flat-square&logo=python&logoColor=FFD43B"
  alt="Python"
/>
<img
  src="https://img.shields.io/badge/REST_APIs-202B36?style=flat-square&logo=swagger&logoColor=85EA2D"
  alt="REST APIs"
/>
<img
  src="https://img.shields.io/badge/LLM_Integration-202B36?style=flat-square&logo=openai&logoColor=white"
  alt="LLM Integration"
/>
<img
  src="https://img.shields.io/badge/Embeddings-202B36?style=flat-square&logo=huggingface&logoColor=FFD21E"
  alt="Embeddings"
/>
<img
  src="https://img.shields.io/badge/Gmail_API-202B36?style=flat-square&logo=gmail&logoColor=EA4335"
  alt="Gmail API"
/>
<img
  src="https://img.shields.io/badge/Google_Sheets_API-202B36?style=flat-square&logo=googlesheets&logoColor=34A853"
  alt="Google Sheets API"
/>
<img
  src="https://img.shields.io/badge/Async_Workflows-202B36?style=flat-square&logo=python&logoColor=8AB4F8"
  alt="Asynchronous Workflows"
/>

</div>

<br/>

**Impact:** Automated the tracking of **100+ job listings** and improved repeated data-collection and matching workflows through asynchronous processing.

---
## `03 // FEATURED BUILDS`

<!-- ==================== RESUMEIQ ==================== -->

<img
  src="https://img.shields.io/badge/ResumeIQ-315A76?style=for-the-badge&logo=probot&logoColor=white"
  alt="ResumeIQ"
/>

<div align="center">

<img
  src="./assets/resumeiq.svg"
  width="94%"
  alt="ResumeIQ system architecture"
/>

</div>

### AI-Powered Hiring Intelligence Platform

A full-stack recruitment platform combining resume screening, semantic candidate matching, privacy-aware evaluation, AI-generated interviews, skill-gap analysis, and recruiter dashboards.

**Engineering highlights**

- Built semantic resume matching using **Sentence Transformers and FAISS**
- Developed candidate search, comparison, analytics, and recommendation workflows
- Added AI-generated interviews using **Groq LLaMA-3**
- Implemented identity-aware preprocessing before candidate evaluation
- Built separate candidate and recruiter experiences through a full-stack architecture

### Technologies Used

<div align="left">

<img src="https://img.shields.io/badge/React-1D2F42?style=flat-square&logo=react&logoColor=61DAFB" alt="React" />
<img src="https://img.shields.io/badge/Vite-1D2F42?style=flat-square&logo=vite&logoColor=BD8BFF" alt="Vite" />
<img src="https://img.shields.io/badge/Node.js-1D2F42?style=flat-square&logo=nodedotjs&logoColor=5FA04E" alt="Node.js" />
<img src="https://img.shields.io/badge/Express.js-1D2F42?style=flat-square&logo=express&logoColor=white" alt="Express.js" />
<img src="https://img.shields.io/badge/MongoDB-1D2F42?style=flat-square&logo=mongodb&logoColor=47A248" alt="MongoDB" />
<img src="https://img.shields.io/badge/Python-1D2F42?style=flat-square&logo=python&logoColor=FFD43B" alt="Python" />
<img src="https://img.shields.io/badge/FAISS-1D2F42?style=flat-square" alt="FAISS" />
<img src="https://img.shields.io/badge/Sentence_Transformers-1D2F42?style=flat-square&logo=huggingface&logoColor=FFD21E" alt="Sentence Transformers" />
<img src="https://img.shields.io/badge/Groq_LLaMA--3-1D2F42?style=flat-square" alt="Groq LLaMA-3" />

</div>

<br/>

<a href="https://github.com/shreya-osr5513/ResumeIQ"><img src="https://img.shields.io/badge/View_Repository-171D25?style=for-the-badge&logo=github&logoColor=white" alt="ResumeIQ repository"></a>
&nbsp;
<a href="YOUR_RESUMEIQ_LIVE_DEMO_LINK"><img src="https://img.shields.io/badge/Live_Demo-315A76?style=for-the-badge&logo=vercel&logoColor=white" alt="ResumeIQ live demo"></a>

---

<!-- ==================== HIREENGINE ==================== -->

<img
  src="https://img.shields.io/badge/HireEngine-925B4A?style=for-the-badge&logo=fastapi&logoColor=white"
  alt="HireEngine"
/>

<div align="center">

<img
  src="./assets/hireengine.svg"
  width="94%"
  alt="HireEngine system architecture"
/>

</div>

### Automated Job Aggregation & Alert System

A production-style backend system that automates job discovery, structured storage, personalized skill matching, scheduled processing, and notification delivery.

**Engineering highlights**

- Built a job-ingestion layer that fetches and structures opportunities from external sources
- Designed normalized storage using **SQLite and SQLAlchemy ORM**
- Implemented skill-based matching with an extensible, NLP-ready architecture
- Added scheduled asynchronous workflows using **APScheduler**
- Prevented duplicate jobs and duplicate notifications through idempotent processing
- Exposed both **REST and GraphQL** interfaces
- Delivered matched-job alerts through an SMTP-based email system

### Technologies Used

<div align="left">

<img src="https://img.shields.io/badge/FastAPI-352B2A?style=flat-square&logo=fastapi&logoColor=9CD9CD" alt="FastAPI" />
<img src="https://img.shields.io/badge/Python-352B2A?style=flat-square&logo=python&logoColor=FFD43B" alt="Python" />
<img src="https://img.shields.io/badge/SQLite-352B2A?style=flat-square&logo=sqlite&logoColor=8FC7E8" alt="SQLite" />
<img src="https://img.shields.io/badge/SQLAlchemy-352B2A?style=flat-square&logo=sqlalchemy&logoColor=E26A4F" alt="SQLAlchemy" />
<img src="https://img.shields.io/badge/APScheduler-352B2A?style=flat-square&logo=clockify&logoColor=F0A17B" alt="APScheduler" />
<img src="https://img.shields.io/badge/BeautifulSoup-352B2A?style=flat-square&logo=python&logoColor=EBC38D" alt="BeautifulSoup" />
<img src="https://img.shields.io/badge/REST_API-352B2A?style=flat-square&logo=swagger&logoColor=85EA2D" alt="REST API" />
<img src="https://img.shields.io/badge/GraphQL-352B2A?style=flat-square&logo=graphql&logoColor=EC7EBB" alt="GraphQL" />
<img src="https://img.shields.io/badge/SMTP-352B2A?style=flat-square&logo=gmail&logoColor=EA6A5F" alt="SMTP" />

</div>

<br/>

<a href="https://github.com/shreya-osr5513/HireEngine"><img src="https://img.shields.io/badge/View_Repository-241E1E?style=for-the-badge&logo=github&logoColor=white" alt="HireEngine repository"></a>
&nbsp;
<a href="https://hireengine.onrender.com/"><img src="https://img.shields.io/badge/Live_API_Docs-925B4A?style=for-the-badge&logo=render&logoColor=white" alt="HireEngine live Demo"></a>

---

<!-- ==================== INSIGHTFLOW ==================== -->

<img
  src="https://img.shields.io/badge/InsightFlow-665B91?style=for-the-badge&logo=streamlit&logoColor=white"
  alt="InsightFlow"
/>

<div align="center">

<img
  src="./assets/insightflow.svg"
  width="94%"
  alt="InsightFlow system architecture"
/>

</div>

### AI Business Analytics Dashboard

An interactive business-intelligence application that processes sales data, calculates dynamic KPIs, generates responsive visualizations, and produces AI-assisted business recommendations.

**Engineering highlights**

- Loaded and processed sales data using **Pandas**
- Calculated revenue, profit, category, and regional KPIs dynamically
- Added interactive filtering by region and category
- Built responsive sales and performance visualizations using **Plotly**
- Integrated an AI layer for recommendations based on the selected dashboard context
- Deployed the application using **Streamlit Cloud**

### Technologies Used

<div align="left">

<img src="https://img.shields.io/badge/Python-29283A?style=flat-square&logo=python&logoColor=FFD43B" alt="Python" />
<img src="https://img.shields.io/badge/Streamlit-29283A?style=flat-square&logo=streamlit&logoColor=FF6B6B" alt="Streamlit" />
<img src="https://img.shields.io/badge/Pandas-29283A?style=flat-square&logo=pandas&logoColor=C8B6FF" alt="Pandas" />
<img src="https://img.shields.io/badge/Plotly-29283A?style=flat-square&logo=plotly&logoColor=9F92FF" alt="Plotly" />
<img src="https://img.shields.io/badge/OpenAI_API-29283A?style=flat-square&logo=openai&logoColor=A8D8C4" alt="OpenAI API" />
<img src="https://img.shields.io/badge/Streamlit_Cloud-29283A?style=flat-square&logo=streamlit&logoColor=FF6B6B" alt="Streamlit Cloud" />

</div>

<br/>

<a href="https://github.com/shreya-osr5513/Insight-Flow"><img src="https://img.shields.io/badge/View_Repository-211F2D?style=for-the-badge&logo=github&logoColor=white" alt="InsightFlow repository"></a>
&nbsp;
<a href="https://ai-business-dashboard-shreyagupta.streamlit.app"><img src="https://img.shields.io/badge/Live_Dashboard-665B91?style=for-the-badge&logo=streamlit&logoColor=white" alt="InsightFlow live dashboard"></a>

---

## `04 // OPEN SOURCE`

### Harbor Framework — PR #1140

Contributed a Dockerized data-aggregation pipeline to the **Harbor Framework**, processing **9,994 rows** of Kaggle Superstore data.

- Built a reproducible data-processing workflow
- Packaged the pipeline using Docker
- Submitted contribution through **PR #1140**
- Contribution was reviewed and merged into production

### Technologies Used

<div align="left">

<img src="https://img.shields.io/badge/Docker-202B36?style=flat-square&logo=docker&logoColor=2496ED" alt="Docker" />
<img src="https://img.shields.io/badge/Python-202B36?style=flat-square&logo=python&logoColor=FFD43B" alt="Python" />
<img src="https://img.shields.io/badge/Data_Pipeline-202B36?style=flat-square&logo=apacheairflow&logoColor=017CEE" alt="Data Pipeline" />
<img src="https://img.shields.io/badge/GitHub_PR-202B36?style=flat-square&logo=github&logoColor=white" alt="GitHub pull request" />

</div>

<br/>

<a href="#">
  <img
    src="https://img.shields.io/badge/View_Merged_PR-171D25?style=for-the-badge&logo=github&logoColor=white"
    alt="View merged pull request"
  />
</a>

---

## `05 // TECHNOLOGY MATRIX`

### Languages

<div align="left">

<img src="https://skillicons.dev/icons?i=cpp,python&theme=dark" alt="C++ and Python" />
&nbsp;
<img src="https://img.shields.io/badge/SQL-202B36?style=for-the-badge&logo=postgresql&logoColor=white" alt="SQL" />

</div>

### Backend & APIs

<div align="left">

<img src="https://skillicons.dev/icons?i=fastapi,nodejs,express&theme=dark" alt="FastAPI, Node.js and Express.js" />
&nbsp;
<img src="https://img.shields.io/badge/REST_APIs-202B36?style=for-the-badge&logo=swagger&logoColor=85EA2D" alt="REST APIs" />
<img src="https://img.shields.io/badge/GraphQL-202B36?style=for-the-badge&logo=graphql&logoColor=E10098" alt="GraphQL" />

</div>

### Databases

<div align="left">

<img src="https://skillicons.dev/icons?i=mongodb,mysql,sqlite&theme=dark" alt="MongoDB, MySQL and SQLite" />

</div>

### AI Engineering

<div align="left">

<img src="https://img.shields.io/badge/LLM_Integration-202B36?style=for-the-badge&logo=openai&logoColor=white" alt="LLM Integration" />
<img src="https://img.shields.io/badge/Embeddings-202B36?style=for-the-badge&logo=huggingface&logoColor=FFD21E" alt="Embeddings" />
<img src="https://img.shields.io/badge/FAISS-202B36?style=for-the-badge&logoColor=white" alt="FAISS" />
<img src="https://img.shields.io/badge/Sentence_Transformers-202B36?style=for-the-badge&logo=huggingface&logoColor=FFD21E" alt="Sentence Transformers" />

</div>

### Tools

<div align="left">

<img src="https://skillicons.dev/icons?i=git,github,docker,vscode&theme=dark" alt="Git, GitHub, Docker and VS Code" />

</div>

---

## `06 // ENGINEERING PROCESS`

> **01 — Understand**  
> Study the workflow, requirements, constraints, and expected outcome.

> **02 — Design**  
> Plan the data model, API layer, components, and system boundaries.

> **03 — Build**  
> Implement the smallest reliable version of the system.

> **04 — Measure**  
> Observe performance, failures, user experience, and output quality.

> **05 — Improve**  
> Refine the architecture, remove bottlenecks, and ship better iterations.

---

## `07 // GITHUB SIGNALS`

<div align="center">

<img
  width="80%"
  src="https://streak-stats.demolab.com?user=shreya-osr5513&hide_border=true&background=08111F&ring=82AFC2&fire=8EC3B2&currStreakLabel=82AFC2&sideLabels=EAF7FF&dates=8CA3B7&currStreakNum=EAF7FF&sideNums=EAF7FF"
  alt="GitHub contribution streak"
/>

<br/>

<img
  width="95%"
  src="https://github-readme-activity-graph.vercel.app/graph?username=shreya-osr5513&bg_color=08111F&color=EAF7FF&line=82AFC2&point=8EC3B2&area=true&hide_border=true"
  alt="GitHub contribution activity graph"
/>

</div>

---

## `08 // CONNECT`

<div align="center">

### Let’s stay connected.

Building, learning, and improving one system at a time.

<br/>

<a href="https://www.linkedin.com/in/shreya-gupta-7b6b96250">
  <img
    src="https://img.shields.io/badge/LinkedIn-shreya--gupta-315A76?style=for-the-badge&logo=linkedin&logoColor=white"
    alt="LinkedIn"
  />
</a>
&nbsp;
<a href="https://github.com/shreya-osr5513">
  <img
    src="https://img.shields.io/badge/GitHub-shreya--osr5513-171D25?style=for-the-badge&logo=github&logoColor=white"
    alt="GitHub"
  />
</a>
&nbsp;
<a href="mailto:shreya.gupta.osr@gmail.com">
  <img
    src="https://img.shields.io/badge/Email-Let's_Connect-76594F?style=for-the-badge&logo=gmail&logoColor=white"
    alt="Email"
  />
</a>

<br/><br/>

<img
  src="https://komarev.com/ghpvc/?username=shreya-osr5513&style=flat-square&color=315A76&label=PROFILE+VIEWS"
  alt="Profile views"
/>

</div>

</div>
